var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span class="overlayThing">You have </span>
		<h2  class="overlayThing" id="points">{{format(player["tree-tab"].jellykens)}}</h2>
		<span class="overlayThing"> total jellykens</span>
		<br>
		<span v-if="canGenPoints()"  class="overlayThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen())}} jellies/sec)</span>
		<span v-if="canGenKens()"  class="overlayThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getKenPerMin())}} jellykens/min)</span>
